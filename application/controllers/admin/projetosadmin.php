...
class ProjetosAdmin extends Controller
{
    ...
    public function main()
    {
        ...
        $dados["listagem"] = $this->modelProjetos->getAllProjetosCategorias();

        //Carregando a view
        $this->loadView("admin/projetos/main", $dados);
    }
    ...
    public function cadastrar()
    {
        ...
        $dados["galeria"] = array();
        $dados["urlAction"] = SITE_URL . "/projetos-admin/cadastrar-exe";
        $dados["urlCategoria"] = SITE_URL . "/projetos-admin/categoria-cadastrar-exe";
        $dados["urlCategoriaRemover"] = SITE_URL . "/projetos-admin/categoria-remover-exe";

        //Categorias do projeto
        $dados["categoria"] = $this->modelProjetos->getAllCategoria(array("orderBy" => "titulo ASC"));

        //Carrega visão
        $this->loadView("admin/projetos/cadUpdate", $dados);
    }
    ...
    public function cadastrarExe()
    {
        ...
        //Validando dados
        $arrRetorno = $this->modelProjetos->validaForm($_POST);
        ...
        if ($arrRetorno["tipo"] == "sucesso") {
            ...
            $idSave = $this->modelProjetos->save($dataPost);

            //Cadastra Slug no banco
            $controladorBase = Common::slug("Projetos Detalhes");
            ...
            //grava log
            $this->modelLog->gravarLog("Cadastrado novo projeto " . $dataPost['titulo']);

            //Url editar projetos
            $url = SITE_URL . "/projetos-admin/editar/" . $idSave;

            //grava Notificação
            $this->modelNotificacao->gravarNotificacao("Cadastrado novo projeto " . $dataPost["titulo"], $url, "all");
        }
    ...
    public function categoriaCadastrarExe()
    {
        ...
        //Cadastra dados no banco
        $idSave = $this->modelProjetos->saveCategoria($dataPost);
    ...
    public function categoriaRemoverExe()
    {
        ...
        //Removendo categoria
        $this->modelProjetos->deleteCategoria($id);
    ...
    public function editar($id = 0)
    {
        ...
        //Carregando dados da view
        $dados["dadosBanco"] = $this->modelProjetos->getByid($id);
        $dados["breadcrumb"] = array(
            array(
                "nome" => "Projetos",
                "url" => SITE_URL . "/projetos-admin",
        ...
        $dados["urlAction"] = SITE_URL . "/projetos-admin/editar-exe/" . $dados["dadosBanco"]["id"];
        $dados["urlCategoria"] = SITE_URL . "/projetos-admin/categoria-cadastrar-exe";
        $dados["urlCategoriaRemover"] = SITE_URL . "/projetos-admin/categoria-remover-exe";
    ...
    public function editarExe($id = 0)
    {
        ...
        $dadosBanco = $this->modelProjetos->getByid($id);            

        //Validando dados
        if (empty($_FILES)) {
            $arrRetorno = $this->modelProjetos->validaForm($_POST, "edicao", $dadosBanco["titulo"]);
        } else {
            $arrRetorno = $this->modelProjetos->validaForm($_POST, "edicaoImagem",$dadosBanco["titulo"]);
        }
        ...
        if ($arrRetorno["tipo"] == "sucesso") {
            ...
            //Cadastra dados no banco
            $this->modelProjetos->update($dataPost, $id);
            ...
            //grava log
            $this->modelLog->gravarLog("Editado projeto " . $dataPost['titulo']);

            //Url da notificação
            $url = SITE_URL . "/projetos-admin/editar/" . $id;

            //grava Notificação
            $this->modelNotificacao->gravarNotificacao("Editado projeto no sistema " . $dataPost["titulo"], $url, "all");
        }
    ...
    public function ativarRegistro($id, $campoBanco = "ativo")
    {
        ...
        $projetos = $this->modelProjetos->getByid($id);

        //Faz a inversão do botão
        $ativo = ($projetos[$campoBanco] == "S") ? "N" : "S";
    ...
    public function remover($id)
    {
        ...
        $dados = $this->modelProjetos->getByid($id);
        ...
        if ($this->modelProjetos->delete($id)) {
            ...
            //grava log
            $this->modelLog->gravarLog("Excluído projeto " . $dados["titulo"] . " - " . $dados["id"]);

            //Notificação
            $this->modelNotificacao->gravarNotificacao("Excluído projeto " . $dados["titulo"] . " - " . $dados["id"], "", "all");
    ...
    public function lerDocumentoModal($id)
    {
        ...
        if (count($arrGaleria) >= 1) {
            $img = "";
            $caminhoImagem = STATIC_URL . "/img/projetos/{$dadosBanco['dir_galeria']}";
        ...
        $arrRetorno = array(
            ...
            "editar"    => SITE_URL . "/projetos-admin/editar/$id",
            "galeria"   => $arrGaleria,
        );
