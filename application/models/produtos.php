...
class Produtos extends Model {
     ...
     * @return array
     */
    public function getAllProdutosCategoria($config = array("where" => "", "orderBy" => "", "limit" => "", "offset" => "")) {
        $query = "SELECT p.id, p.titulo, p.texto_curto, p.texto, p.dir_galeria, p.capa, pc.cor_bloco, pc.cor_texto FROM {$this->_tabela} p INNER JOIN produtos_categoria pc 
ON p.categoria_id = pc.id ";
    ...
    public function validaForm($dataCadastro, $tipoValidacao = "cadastro"){
        ...
        if(Common::validarEmBranco($dataCadastro["titulo"])){
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "<strong>Título</strong> não pode estar em branco",
                "iconAlert" => "fa-user-tie",
            );
        }elseif(Common::validarMaximo($dataCadastro["texto_curto"],250)){
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "<strong>Texto Curto</strong> não pode ter mais de 100 caracteres",
                "iconAlert" => "fa-user-tie",
            );
        }elseif(Common::validarMaximo($dataCadastro["texto"],5000)){
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "<strong>Texto</strong> não pode ter mais de 500 caracteres",
                "iconAlert" => "fa-user-tie",
            );
        }else{
