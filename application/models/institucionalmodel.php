...
class InstitucionalModel extends Model {
    ...
    private $_tabela = "site_institucional";
    ...
    public function getLike($busca) {
        $query = "SELECT * FROM {$this->_tabela} WHERE texto LIKE '%$busca%' OR missao LIKE '%$busca%' OR visao LIKE '%$busca%' OR valores LIKE '%$busca%'";
        return $this->db->select($query);
    }
    ...
    public function validaForm($dataCadastro, $tipoValidacao = "cadastro"){
        ...
        if(Common::validarEmBranco($dataCadastro["texto"])){
            $arrRetorno = array(
                "tipo" => "error",
