...
class SeoModel extends Model
{
    ...
    public function getByString($url)
    {
        $id = (string) $url; //recebendo apenas string
        $query = "SELECT * FROM {$this->_tabela} WHERE url = :id";
        $dataWhere = array(":id" => $id);
        return $this->db->select($query, $dataWhere, FALSE);
    }
    ...
    {
        $query = "SELECT * FROM {$this->_tabela} WHERE titulo LIKE '%$busca%' OR descricao LIKE '%$busca%' ORDER BY id DESC LIMIT 100";
        return $this->db->select($query);
    }
    ...
    public function validaForm($dataCadastro, $tipoValidacao = "cadastro")
    {
        ...
        if (Common::validarEmBranco($dataCadastro["url"])) {
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "<strong>URL</strong> não pode estar em branco",
            );
        
        } else if (Common::validarMinimo($dataCadastro["titulo"],30)) {
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "<strong>Título</strong> deve ter no mínimo 30 caracteres",
            );
        }else if (Common::validarMaximo($dataCadastro["titulo"],70)) {
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "<strong>Título</strong> deve ter no máximo 70 caracteres",
            );
        } else if (Common::validarMinimo($dataCadastro["descricao"],60)) {
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "<strong>Descrição</strong> deve ter no mínimo 60 caracteres",
            );
        } else if (Common::validarMaximo($dataCadastro["descricao"],160)) {
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "<strong>Descrição</strong> deve ter no máximo 160 caracteres",
            ); 
        } else {
