...
class Slug extends Model {
    ...
    public function getBySlug($slug) {
        $query = "SELECT * FROM {$this->_tabela} WHERE slug = :id";
        $dataWhere = array(":id" => $slug);
        return $this->db->select($query, $dataWhere, FALSE);
    }
    ...
    public function validaForm($dataCadastro, $apelidoAtual){
        ...
        //Verificando se apelido não está cadastrado no sistema ou nas palavras reservadas 
        ...
        $arrPalavrasReservadas = array("index","api","siteemmanutencao",
        "Site em Manutenção", "site em manutencao","Site em manutencao",
        "pag","notfound","busca","Site em manutenção", "site em manutenção");
        ...
        } else if(count($dadosMenuSite) >= 1){
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "Esse apelido: <strong>{$dataCadastro['pseudo_titulo']}</strong> não pode ser cadastrado, escolha outro nome!",
            );
        } else if(count($dadosMenuSistema) >= 1){
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "Esse apelido: <strong>{$dataCadastro['pseudo_titulo']}</strong> não pode ser cadastrado, escolha outro nome!",
            );
        } else if(is_int(array_search($dataCadastro["pseudo_titulo"],$arrPalavrasReservadas))){
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "Esse apelido: <strong>{$dataCadastro['pseudo_titulo']}</strong> não pode ser cadastrado, escolha outro nome!",
            );
