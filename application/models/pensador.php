...
class Pensador extends Model {
    ...
    public function getFraseMotivacional(){
        $query = "SELECT * FROM {$this->_tabela} ORDER BY RAND() LIMIT 1";
    ...
    public function getLike($busca) {
        $query = "SELECT * FROM {$this->_tabela} WHERE frase LIKE '%$busca%' OR autor LIKE '%$busca%' ORDER BY RAND() LIMIT 1";
        return $this->db->select($query);
    }
