...
class Configuracao extends Model {
    ...
    private $_tabela = "config_sistema";
    ...
    public function siteEmManutenção(){
        $config = $this->getByid(1);
        if($config["site_construcao"] == 'S'){
            return true;
        }else{
            return false;
        }
     ...
     * @return Array
     */
    public function getConfigSistema($id = 1) {
        $query = "SELECT * FROM {$this->_tabela} WHERE id = :id";
        $dataWhere = array(":id" => $id);
        return $this->db->select($query, $dataWhere, FALSE);
    }
     ...
     * @return Integer
     */
    public function updateSistema($dataPost, $id) {
        //montando o where do update
        $where = "id = " . (int) $id;
    ...
    public function validaForm($dataCadastro, $tipoValidacao = "cadastro"){
        ...
        if(Common::validarEmBranco($dataCadastro["nome_fantasia"])){
            $arrRetorno = array(
                "tipo" => "error",
                "mensagem" => "<strong>Nome Fantasia</strong> não pode estar em branco",
            );
        }else{
